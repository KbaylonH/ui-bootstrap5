...
module.exports = function(grunt) {
    ...
    grunt.registerTask('build', 'Create bootstrap build files', function() {
        ...
        grunt.task.run(['concat', 'uglify', 'makeModuleMappingFile', 'makeRawFilesJs', 'makeVersionsMappingFile', 'aliasDist']);
    });

    // The package's own root index.js does require('./dist/ui-bootstrap-tpls')
    // -- i.e. these unversioned files, not the versioned ones above, are what
    // consumers actually get via require('ui-bootstrap4'). Previously the
    // only thing that (re)created them was shell:publish, as part of a full
    // rm -rf dist/* + npm publish release -- far too heavy just to pick up
    // a local build. Runs automatically at the end of `build`, and can also
    // be run on its own (e.g. `grunt aliasDist`) to refresh just these two
    // files without rebuilding everything else.
    grunt.registerTask('aliasDist', 'Copy the versioned js bundles to the unversioned aliases required by index.js', function() {
        const dist = grunt.config('dist');
        const filename = grunt.config('filename');
        const version = grunt.config('pkg').version;

        [filename, `${filename}-tpls`].forEach((base) => {
            const src = `${dist}/${base}-${version}.js`;
            const dest = `${dist}/${base}.js`;
